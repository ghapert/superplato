...
LOG_PATH = os.path.join(LOG_DIR, "auto_attendance_worker_part2.log")
...
async def run_auto_attendance_for_user_lecture(user_id, lecture_id):
    ...
    try:
        ...
        if not user or not lecture:
            logging.error(f"[worker_part2] 잘못된 user_id={user_id} 또는 lecture_id={lecture_id}")
            return

        # 이미 오늘 출석이 있다면 스킵
        already = already_attended_today(db, user.id, lecture.id)
        if already:
            logging.info(f"[worker_part2] 이미 출석됨 user={user.id}, lecture={lecture.id}")
            return
        ...
        async with async_playwright() as p:
            try:
                ...
                if "autoattendance.php" not in page.url:
                    logging.info(f"[worker_part2] 현재 출석 가능 상태 아님, url={page.url}")
                    await page.close()
                    await browser.close()
                    return
                ...
                # 파트2 범위
                codes = list(range(334, 667))  # 334 ~ 666
                ...
                for code in codes:
                    try:
                        ...
                        if "my_status.php" in res.url:
                            ...
                            if already2:
                                logging.info(f"[worker_part2] 동시에 출석완료됨 user={user.id}, lecture={lecture.id}")
                                return
                            ...
                            try:
                                ...
                                db.rollback()
                                logging.warning(
                                    f"[worker_part2] 중복 출석 IntegrityError user={user.id}, lec={lecture.id}, code={code}"
                                )
                                return
                            logging.info(
                                f"[worker_part2] 자동 출석 성공 user={user.id}, lecture={lecture.id}, code={code:03d}"
                            )
                            return

                    except Exception as e:
                        logging.warning(f"[worker_part2] 브루트포스 시도 실패 code={code:03d}, err={str(e)}")

                logging.warning(f"[worker_part2] 모든 코드 실패 user={user.id}, lecture={lecture.id}")
                await browser.close()

            except Exception as e:
                logging.error(f"[worker_part2] Playwright 작업 실패 user={user_id}, lec={lecture_id}, {str(e)}")
...
def main():
    ...
    logging.info(f"[worker_part2 main] Start: user_id={args.user_id}, lecture_id={args.lecture_id}")

    import asyncio
    asyncio.run(run_auto_attendance_for_user_lecture(args.user_id, args.lecture_id))

    logging.info("[worker_part2 main] Done.")
